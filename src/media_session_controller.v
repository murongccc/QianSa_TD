module media_session_controller #(
    ...
    input wire frame_commit, input wire load_abort,
    // The video domain toggles this only after it has consumed the requested
    // slot on a frame boundary.  It is synchronized below before use.
    input wire display_switch_applied_toggle_async,
    output reg load_start, output reg [1:0] load_media_index,
    output reg [1:0] write_slot, output reg [1:0] display_slot,
    output reg display_switch_toggle,
...
reg [2:0] slot_ready;
reg [2:0] slot_failed;
reg display_switch_inflight;
reg display_switch_applied_meta;
reg display_switch_applied_sync;
reg display_switch_applied_seen;
...
wire key_next_press;
reg key_auto_meta, key_auto_sync;
...
wire manual_request = key_next_press ||
                      (uart_command_event && uart_command_sync == UART_CMD_NEXT);
...
function [1:0] first_ready_slot;
    input [2:0] ready;
    begin
        if (ready[0]) first_ready_slot = 2'd0;
        else if (ready[1]) first_ready_slot = 2'd1;
        else first_ready_slot = 2'd2;
    end
endfunction

function [1:0] next_ready_slot;
    input [1:0] current;
    input [2:0] ready;
    begin
        if (current == 2'd0 && ready[1]) next_ready_slot = 2'd1;
        else if (current != 2'd2 && ready[2]) next_ready_slot = 2'd2;
        else if (ready[0]) next_ready_slot = 2'd0;
        else if (ready[1]) next_ready_slot = 2'd1;
        else next_ready_slot = 2'd2;
    end
endfunction

wire have_ready_slot = |slot_ready;
wire [1:0] requested_next_slot = next_ready_slot(display_slot, slot_ready);
wire manual_switch_start = display_valid && !display_switch_inflight &&
                           (manual_next_pending != 3'd0) &&
                           (requested_next_slot != display_slot);
wire auto_switch_start = display_valid && !display_switch_inflight &&
                         !manual_switch_start && auto_next_pending &&
                         (requested_next_slot != display_slot);
...
always @(posedge clk or posedge rst) begin
    if (rst) begin
        ...
        slot_ready<=3'b000; slot_failed<=3'b000; display_switch_inflight<=1'b0;
        display_switch_applied_meta<=1'b0; display_switch_applied_sync<=1'b0;
        display_switch_applied_seen<=1'b0;
        uart_command_meta<=3'd0; uart_command_sync<=3'd0;
        uart_toggle_meta<=1'b0; uart_toggle_sync<=1'b0; uart_toggle_seen<=1'b0;
        key_auto_meta<=1'b0; key_auto_sync<=1'b0;
    end else begin
        ...
        // SW2 is an active-high level switch (the board constraint uses a
        // pulldown).  Synchronize the level directly; treating it as a
        // button pulse made automatic playback randomly disable or never
        // start while a frame was loading.
        key_auto_meta <= key_auto;
        key_auto_sync <= key_auto_meta;
        display_switch_applied_meta <= display_switch_applied_toggle_async;
        display_switch_applied_sync <= display_switch_applied_meta;
        load_start<=1'b0;
        if (!scan_done || media_count==3'd0) begin
            display_valid<=1'b0; auto_play_enabled<=1'b0; load_inflight<=1'b0;
            auto_count<=32'd0; manual_next_pending<=3'd0; auto_next_pending<=1'b0;
            slot_ready<=3'b000; slot_failed<=3'b000; display_switch_inflight<=1'b0;
            display_slot<=2'd0; display_switch_toggle<=1'b0;
            display_switch_applied_seen<=display_switch_applied_sync;
            ...
            if (manual_request) begin
                if (manual_switch_start) begin
                    // A press coincident with a displayed switch replaces the
                    // consumed request, so rapid presses remain lossless.
            ...
            end else if (manual_switch_start) begin
                manual_next_pending <= manual_next_pending - 3'd1;
            end
            ...
            // SW2 is a level input in the board design.  Latch its state for
            // the carousel, but do not overwrite a UART toggle command in the
            // same clock; the old unconditional assignment could cancel a
            // just-accepted command and made the mode appear disabled.
            if (!(uart_command_event && uart_command_sync == UART_CMD_AUTO_TOGGLE))
                auto_play_enabled <= key_auto_sync && (media_count > 3'd1);
            if ((uart_command_event && uart_command_sync == UART_CMD_AUTO_TOGGLE) ||
                         (uart_command_event && (uart_command_sync >= UART_CMD_PERIOD_1S))) begin
                // The command handler above has already reset auto_count.
            end else if (key_auto_sync && display_valid && !display_switch_inflight && media_count>3'd1) begin
                if (auto_due) begin auto_count<=32'd0; auto_next_pending<=1'b1; end
                else auto_count<=auto_count+32'd1;
            end else auto_count<=32'd0;

            // Completion of a displayed-slot request is acknowledged from the
            // video domain only at frame_start.  Do not issue another slot
            // update before that acknowledgement, otherwise a fast SD-clock
            // sequence can be collapsed by the two-flop video synchronizer.
            if (display_switch_inflight &&
                (display_switch_applied_sync != display_switch_applied_seen)) begin
                display_switch_applied_seen <= display_switch_applied_sync;
                display_switch_inflight <= 1'b0;
                display_valid <= 1'b1;
            end

            if (load_inflight && load_abort) begin
                // Drop a malformed/truncated BMP without disturbing the
                // currently displayed frame.  During initial loading there
                // is no displayed frame, so advance the catalogue index
                // directly to avoid retrying the same bad file forever.
                load_inflight<=1'b0;
                slot_failed[write_slot] <= 1'b1;
            end else if (load_inflight && frame_commit) begin
                slot_ready[write_slot] <= 1'b1;
                load_inflight <= 1'b0;
            end

            // First visible frame never waits for the other two BMP files.
            // Thereafter all slot updates are display-only operations.
            if (!display_valid && !display_switch_inflight && have_ready_slot) begin
                display_slot <= first_ready_slot(slot_ready);
                display_switch_toggle <= ~display_switch_toggle;
                display_switch_inflight <= 1'b1;
            end else if (manual_switch_start || auto_switch_start) begin
                display_slot <= requested_next_slot;
                display_switch_toggle <= ~display_switch_toggle;
                display_switch_inflight <= 1'b1;
                if (auto_switch_start)
                    auto_next_pending <= 1'b0;
            end

            // Keep each discovered source permanently assigned to its own
            // SDRAM frame slot.  A ready frame is therefore never re-read
            // from SD merely because the user changes picture.
            if (!load_inflight && loader_ready) begin
                if (!slot_ready[0] && !slot_failed[0]) begin
                    load_media_index<=2'd0; write_slot<=2'd0; load_start<=1'b1; load_inflight<=1'b1;
                end else if (media_count > 3'd1 && !slot_ready[1] && !slot_failed[1]) begin
                    load_media_index<=2'd1; write_slot<=2'd1; load_start<=1'b1; load_inflight<=1'b1;
                end else if (media_count > 3'd2 && !slot_ready[2] && !slot_failed[2]) begin
                    load_media_index<=2'd2; write_slot<=2'd2; load_start<=1'b1; load_inflight<=1'b1;
                end
            end
        end
    end
